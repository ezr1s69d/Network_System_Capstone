...
class ClientHandler():
    ...
    def __send_file(self, request, file_path):
        ...
        response_str = response['version'] + " " + response['status'] + "\r\n" + "Content-Type: " + response['headers']['Content-Type'] + "\r\n" + "Content-Length: " + response['headers']['Content-Length'] + "\r\n\r\n"
    ...
    def __send_response(self, request, response):
        ...
        # response_str = ?
        # E.g.,response_str = "HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\nContent-Length: 100\r\n\r\n"
        if response['status'] != "400 Bad Request" or response['status'] != "404 Not Found":
            response_str = response['version'] + " " + response['status'] + "\r\n" + "Content-Type: " + response['headers']['Content-Type'] + "\r\n" + "Content-Length: " + str(response['headers']['Content-Length']) + "\r\n\r\n"
        else:
            response_str = response['version'] + " " + response['status'] + "\r\n" + "Content-Type: " + response['headers']['Content-Type'] + "\r\n" + "Content-Length: " + "\r\n\r\n"
