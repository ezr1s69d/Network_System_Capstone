...
class Frame:
    ...
    def to_bytes(self):
        ...
        # bytes = ?
        # bytes = struct.pack(f"{24}sBB{1}s{31}s{self.length}s", self.length, self.type, self.flags, self.r, self.stream_id, self.payload)
        length_type = (self.length << 8) + self.type
        flags = self.flags
        r_id = (self.r << 31) + self.stream_id
        bytes = struct.pack(f"!LBL{self.length}s", length_type, flags, r_id, self.payload)
        return bytes
...
def bytes_to_frames(data):
    ...
    while len(data) > 0:
        ...
        if len(data) >= 6 + length:
            ...
            type, flags, r_stream_id, payload = struct.unpack(f"!BBL{length}s", data[3 : 3 + (6 + length)])
