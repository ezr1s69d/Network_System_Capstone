...
def parse_response_2(stream_id, raw_bytes):
    ...
    for i in range(len(lines) - 1):
        str = lines[i].split(b": ")
        headers[str[0][: len(str[0])].lower().decode()] = str[1].decode()
    # Check valid or not
...
def parse_request_2(stream_id, raw_bytes):
    ...
    for i in range(len(lines) - 1):
        str = lines[i].split(b" ")
        headers[str[0][: len(str[0]) - 1].lower().decode()] = str[1].decode()
...
def parse_response(client_socket, stream):
    ...
    str = lines[0].split(b" ", 1)
    response.version = str[0].decode()
    response.status = str[1].decode()
...
def parse_request(client_socket):
    ...
    str = []
    str = lines[0].split(b" ")
    request.method = str[0].decode()
    resource = str[1].decode()
    request.version = str[2].decode()
    ...
    for i in range(1, 100):
        if lines[i] == b'':
            ...
            str = lines[i].split(b" ")
            headers[str[0][0 : len(str[0]) - 1].decode()] = str[1].decode()
