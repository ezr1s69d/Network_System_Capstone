...
class Setting:
    ...
    def __init__(
        ...
        if max_collision_wait_time is None:
            self.max_collision_wait_time = self.host_num * self.packet_time * coefficient  # TODO: this is not complete, change this (hint: add two parameters)
        else:
            self.max_collision_wait_time = max_collision_wait_time

        if p_resend is None:
            self.p_resend = 1 / (self.host_num * coefficient)  # TODO: this is not complete, change this (hint: add one parameters)
        else:
